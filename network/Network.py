...
class Network:
    ...
    def decoded_message_error_rate_batch_obj(self, messages, decoded_messages):
        error_rates = []
        batch_size = len(messages)
        for i in range(batch_size):
            err = self.decoded_message_error_rate(messages[i], decoded_messages[i])
            error_rates.append(1 - err)
        return torch.tensor(error_rates)  # 返回 Tensor 或者直接返回 list 都可以
