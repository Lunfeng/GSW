...
class JpegBasic(nn.Module):
	...
	def dct(self, image):
		...
		B, C, H, W = image.shape
		assert H % 8 == 0 and W % 8 == 0, "H and W must be divisible by 8 after padding"

		# 先把图片reshape成块
		image = image.view(B, C, H // 8, 8, W // 8, 8)  # 变成 (B, C, H块, 8, W块, 8)
		image = image.permute(0, 2, 4, 1, 3, 5)  # (B, H块, W块, C, 8, 8)
		image = image.reshape(-1, C, 8, 8)  # (B * H块 * W块, C, 8, 8)

		# 对每个8×8块做DCT
		image_dct = torch.matmul(coff, image)
		image_dct = torch.matmul(image_dct, coff.permute(1, 0))

		# reshape回来
		image_dct = image_dct.view(B, H // 8, W // 8, C, 8, 8)
		image_dct = image_dct.permute(0, 3, 1, 4, 2, 5)
		image_dct = image_dct.reshape(B, C, H, W)
	...
	def idct(self, image_dct):
		...
		# 先reshape成块
		image = image_dct.view(B, C, H // 8, 8, W // 8, 8)  # (B, C, H块, 8, W块, 8)
		image = image.permute(0, 2, 4, 1, 3, 5)  # (B, H块, W块, C, 8, 8)
		image = image.reshape(-1, C, 8, 8)  # (B * H块 * W块, C, 8, 8)

		# 对每个8×8块做逆DCT
		image_idct = torch.matmul(coff.permute(1, 0), image)
		image_idct = torch.matmul(image_idct, coff)

		# reshape回来
		image_idct = image_idct.view(B, H // 8, W // 8, C, 8, 8)
		image_idct = image_idct.permute(0, 3, 1, 4, 2, 5)
		image_idct = image_idct.reshape(B, C, H, W)
